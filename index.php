...
function qst_mkdir($path) {
    if (!is_dir($path)) {
        @mkdir($path, 0777, true);
    }
}

function qst_ensure_file($path, $default = '') {
    $dir = dirname($path);
    if (!is_dir($dir)) {
        @mkdir($dir, 0777, true);
    }
...
function qst_ensure_json($path, $default = []) {
    qst_ensure_file($path, json_encode($default, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
}
...
qst_mkdir('EMILS');
qst_mkdir('BUY');
qst_mkdir('assignment');
qst_mkdir('data/id');
qst_mkdir('data/txt');
qst_mkdir('data/api');
$API_KEY= '8759734530:AAEOrjJ9_78Mi3rO3niCqBlwO0e03TJnje8';//توكن بوتك
...
qst_mkdir("data/id/$id");
}
qst_ensure_json("EMILS/$EM/number.json", ["number" => [], "number_my" => 0, "ready_my" => 0]);
qst_ensure_json("EMILS/$EM/send.json", []);
qst_ensure_json("EMILS/$EM/card.json", []);
qst_ensure_json("EMILS/$EM/price.json", []);
qst_ensure_file("EMILS/$EM/rubles.txt", "0");
qst_ensure_file("EMILS/$EM/points.txt", "0");
qst_ensure_file("data/id/$id/restriction.txt", "");
qst_ensure_file("data/id/$id/step.txt", "");
qst_ensure_file("data/id/$id/twas.txt", "");
qst_ensure_file("data/id/$id/number.txt", "");
qst_ensure_file("data/id/$id/monitored.txt", "");
qst_ensure_file("data/id/$id/app.txt", "");
qst_ensure_file("data/id/$id/lift.txt", "");
qst_ensure_file("data/$chat_id/email.txt", "");
