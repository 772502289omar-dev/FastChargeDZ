...
$app = str_replace(["wa","tg","fb","ig","tw","lf","go","im","vi","fu","nf","au","ot"],["ot","tg","fb","ig","tw","tk","go","im","vi","","nf","","ot"],$app);
}
if($site=="smscode"){
...
if($site=="smslive"){
$api = file_get_contents("https://api.alisms.org/stubs/handler_api.php?api_key=$api_key&action=getNumber&service=$app&country=$country");
$explode = explode(":",$api);
$idnumber = $explode[1];
$number = $explode[2];
if($number==null){
...
if($site=="vak"){
$api = json_decode(file_get_contents("https://vak-sms.com/api/getNumber?apiKey=".$api_key."&service=".$app."&country=".$zx),1);
$number  = $api["tel"];
$idnumber = $api["idNum"];
if($number==null){
$status=0;
...
if($site=="acktiwator"){
$api = json_decode(file_get_contents("https://smsak.org/api/getnumber/$api_key?id=$app&code=$zx"),1);
$idnumber = $api['id'];
$number = $api['number'];
if($number==null){
$status=0;
...
if($site=="botsms"){
$api=json_decode(file_get_contents("https://answs1234.darksidehost.com/jj/tele/GetNumber.php?key=$api_key&from_id=6996492261&country=$zx&app=$app"),true);
$idnumber=$api['number'];
$number=$api['number'];
if($number==null){
$status=0;
...
if($site=="grizzly"){
$api=file_get_contents("https://api.grizzlysms.com/stubs/handler_api.php?api_key=$api_key&action=getNumber&service=$app&country=$zx");
$explode = explode(":",$api);
$idnumber = $explode[1];
$number = $explode[2];
if($number==null){
...
if($site=="smscode"){
$api=json_decode(file_get_contents("https://sms-code.ru/api.php?api_key=$api_key&method=phone&service=$app&country=$zx&multiple_sms=0"),1);
...
//تعديل وتصليح موقع العالمي النوبرة -alnwerah
if($site=="duraincloud"){
    $opts = [
        "http" => [
            "timeout" => 10
    ...
    $api_url = "http://api.duraincloud.com/out/ext_api/getMobile?name=$Username&pwd=$Password&ApiKey=$api_key&cuy=$zx&pid=$app&num=1&noblack=0&serial=2";
...
if($site=="smslive"){
$api = file_get_contents("https://api.alisms.org/stubs/handler_api.php?action=getStatus&api_key=$api_key&id=$idnumber");
$ex_api= explode(":",$api);
$code = $ex_api[1];
$agen = 1;
if($code==null){
...
if($site=="acktiwator"){
$api = json_decode(file_get_contents("https://smsak.org/api/getstatus/$api_key?id=$idnumber"),1);
$code = $api['small'];
$agen = 1;
if($code==null){
$status=0;
...
if($site=="botsms"){
$number=trim($number);
$url="https://answs1234.darksidehost.com/jj/tele/GetSms.php?key=$api_key&from_id=6996492261&number=$number";
$response=file_get_contents($url);
$api=json_decode($response,true);
if($api['ok']===true && !empty($api['code'])){
$code=$api['code'];
$status=200;
}else{
$code=null;
...
if($site=="smslive"){
$api = file_get_contents("https://api.alisms.org/stubs/handler_api.php?api_key=$api_key&action=setStatus&api_key=$api_key&id=$idnumber&status=8");
$status=200;
$Location  = "smslive.pro";
}
if($site=="man"){
...
if($site=="acktiwator"){
$api = file_get_contents("http://smsak.org/stubs/handler_api.php?api_key=$api_key&action=setStatus&status=8&id=$idnumber&forward=1");
$status=200;
$Location  = "smsak.org";
}
if($site=="pvapins"){
...
if($site=="smslive"){
$api = json_decode(file_get_contents("https://api.alisms.org/stubs/handler_api.php?api_key=$api_key&action=getPrices&service=$app&country=$country"),1);
...
if($site=="vak"){
$api=json_decode(file_get_contents("https://vak-sms.com/api/getCountNumber/?apiKey=".$api_key."&service=".$app."&country=".$country."&operator=".$operator."&price"),1);
$price=$api["price"];
$add=$api[$app];
if($price!=null){
$status=200;
...
if($site=="acktiwator"){
$api = file_get_contents("https://smsak.org/api/numbersstatus/$api_key?code=$zx");
